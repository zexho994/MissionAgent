...
一个AI驱动的目标达成系统。用户提供简单的目标（如"小红书1个月涨粉1000"），AI负责从战略对齐、团队组建、任务执行到持续优化的全流程。系统像真正的团队一样紧密协作——分析师每日看数据、项目经理协调调整、内容策划响应变化。
...
### 3.1 对话式通信
- [ ] 扩展AgentMessage支持自由文本对话（不只是结构化事件类型）
- [ ] 设计对话上下文：Agent能理解之前的对话历史
- [ ] Agent可以主动发起对话（不限于API触发）

### 3.2 团队协作机制
- [ ] 汇报机制：下级Agent定期向上级汇报进展
- [ ] 请求机制：Agent可以向其他Agent请求信息或协助
- [ ] 通知机制：异常情况主动通知相关负责人
- [ ] 讨论机制：多个Agent围绕一个议题展开讨论

### 3.3 共享上下文
- [ ] 设计团队级别的共享知识库（Mission级记忆）
- [ ] Agent能读取其他Agent的工作产出（Artifact共享）
- [ ] 上下文窗口管理策略（长期任务中保持关键信息不丢失）
...
**验收标准**: 一个为期1个月的小红书运营Mission能从始至终持续运行，中途可以暂停恢复，每周有检查点，最终有完整的执行报告。
